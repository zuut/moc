...
class MocConan(ConanFile):
    ...
    _noCmakeConfigFiles = True #False
    _requirements = [
        "flex/2.6.4",
        "bison/3.7.1"
    ]
    ...
    def source(self):
        ...
        print("Using protocol '%s'" % protocol)
        if protocol == "file" :
            ...
            #Path(dest_moc).mkdir(parents=True, exist_ok=True)
            #Path(dest_uf).mkdir(parents=True, exist_ok=True)
