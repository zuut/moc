...
ufSHConstruct(ufGenericStrHash *lpSelf, int nSize) {
    ufASSERT(lpSelf != 0) if (lpSelf == 0) { return; }
...
ufSHInsert_(ufGenericStrHash *lpSelf, const char *lpszKey,
            ufPtr(void) * lpData) {
    ufASSERT(lpSelf != 0) ufASSERT(lpszKey != 0)
        ufASSERT(lpData != 0) if (lpSelf == 0 || lpszKey == 0 || lpData == 0) {
        return 0;
    }
    ...
    ufCHECK_MEMORY return 1;
}

UF_API(int)
ufSHRemove_(ufGenericStrHash *lpSelf, const char *lpszKey,
            ufPtr(void) * lpData) {
    ufASSERT(lpSelf != 0) ufASSERT(lpszKey != 0) ufASSERT(lpData != 0)
...
ufSHAdd_(ufGenericStrHashIter *i1, ufGenericStrHashIter *i2, int incr) {
    ufASSERT(i1 != 0) ufASSERT(i2 != 0) if (i1 == 0 || i2 == 0) { return; }
...
int main(int argc, char *argv[]) {
    ufStrHash(Foo) myhashTable;
    ufPtr(Foo) myptr;

    myptr = Foo::New(1);
    myhashTable.Insert(myptr->fName, myptr);
    myptr.MakeEmpty();
    ...
    if (myhashTable.Find("Foo(3)", myptr)) {
        cout << "OK: myhashTable.Find(\"Foo(3)\") found " << myptr->fName
             << "\n"
             << flush;
    } else {
    ...
    myhashTable.Remove("Foo(3)");

    if (myhashTable.Find("Foo(3)", myptr)) {
        cout << "ERROR: myhashTable.Find(\"Foo(3)\") "
                "found "
             << myptr->fName << " after it was removed\n"
             << flush;
        myptr.MakeEmpty();
    } else {
        cout << "OK: myhashTable did not find "
                "\"Foo(3)\" after it was removed\n"
             << flush;
    }

    myptr = Foo::New(-4);
    if (myhashTable.Insert("Foo(4)", myptr)) {
