...
ufListRemoveFirst_(ufGenericList *lpSelf, ufPtr(void) * lpDataReturn) {
    ufASSERT(lpDataReturn != 0) if (lpDataReturn == 0) { return 0; }

    ufASSERT(lpSelf != 0) ufCHECK_MEMORY
    ...
    ufCHECK_MEMORY return lpDataReturn;
}
...
ufListNumElements_(ufGenericList *lpSelf) {
    ufASSERT(lpSelf != 0) if (lpSelf == 0) { return 0; }
    ...
    ufCHECK_MEMORY return (i);
}

UF_API(void)
ufListMakeEmpty_(ufGenericList *lpSelf, ufListMakeEmptyFnc lpfnMakeEmpty) {
    ufASSERT(lpSelf != 0) ufASSERT(lpfnMakeEmpty) if (lpSelf == 0) { return; }

    ufCHECK_MEMORY

        ufGenericListIter *lpIter = lpSelf->fFirst;
    ufGenericListIter *lpNext;
    while (lpIter) {
        lpNext = lpIter->fNext;
...
ufListIterAdd_(ufGenericListIter *lpSelf, ufGenericListIter *lpStart,
    ...
    ufASSERT(lpSelf != 0) ufASSERT(lpfnSetPtr != 0)
...
ufListIterAssignList_(ufGenericListIter *lpSelf, const ufGenericList *lpList,
    ...
    ufASSERT(lpSelf != 0) ufASSERT(lpList != 0) ufASSERT(lpfnSetPtr != 0)
...
int main(int argc, char *argv[]) {
    ...
    for (i = mylist; i; i++, e--) {
        ...
        ufASSERT(expected[e] = i.fData->fX) i.fData->Hello();
