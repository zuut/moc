...
int main(int argc, char *argv[]) {
    ufIntHash(Foo) myhashTable;
    ufPtr(Foo) myptr;

    myptr = Foo::New(1);
    myhashTable.Insert(myptr->fX, myptr);
    myptr.MakeEmpty();
    ...
    if (myhashTable.Find(3, myptr)) {
        cout << "OK: myhashTable.Find(3) found " << myptr->fName << "\n"
             << flush;
    } else {
        cout << "ERROR: myhashTable did not find 3\n" << flush;
    }

    myhashTable.Remove(3);

    if (myhashTable.Find(3, myptr)) {
        cout << "ERROR: myhashTable.Find(3) found " << myptr->fName
